...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        ...
        UIImage *image = [[UIImage imageNamed:@"download"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        _saveButton = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:@selector(saveButtonPressed:)];
        UIImage *imagePreview = [[UIImage imageNamed:@"preview"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        _previewButton = [[UIBarButtonItem alloc] initWithImage:imagePreview style:UIBarButtonItemStylePlain target:self action:@selector(previewButtonPressed:)];
        
        int screenHeight = (int)[[UIScreen mainScreen] bounds].size.height;
        NSString *imageName = [NSString stringWithFormat:@"home-%i",screenHeight];
        if([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad){
            imageName = @"home-1024";
        }
        _previewImage = [[UIImageView alloc] initWithFrame:self.view.bounds];
        _previewImage.image = [UIImage imageNamed:imageName];
    }
...
- (void)performLayout {
    ...
    if (_previousButton && _nextButton && numberOfPhotos > 1) {
        ...
        [items addObject:flexSpace];
        [items addObject:_previewButton];
        [items addObject:flexSpace];
    }
    if(_previewImage){
...
- (void)imageSavedToPhotosAlbum:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    NSString *message = @"Save photo success!";
    NSString *title = @"Success";
    if(error){
        message = [NSString stringWithFormat: @"Save photo error: %@", [error description]];
        title = @"Error";
    }
...
- (void)previewButtonPressed:(id)sender{
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        UIAlertAction* lockScreen = [UIAlertAction actionWithTitle:@"Lock Screen Preview" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            NSLog(@"preview lock screen");
            int screenHeight = (int)[[UIScreen mainScreen] bounds].size.height;
            NSString *imageName = [NSString stringWithFormat:@"lock-%i",screenHeight];
            if([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad){
                imageName = @"lock-1024";
            }
            NSLog(@"thong tin: %@",imageName);
            _previewImage.image = [UIImage imageNamed:imageName];
            _previewImage.hidden = false;
            
        }];
        
        UIAlertAction* homeScreen = [UIAlertAction actionWithTitle:@"Home Screen Preview" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            NSLog(@"homeScreen lock screen");
            int screenHeight = (int)[[UIScreen mainScreen] bounds].size.height;
            NSString *imageName = [NSString stringWithFormat:@"home-%i",screenHeight];
            if([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad){
                imageName = @"home-1024";
